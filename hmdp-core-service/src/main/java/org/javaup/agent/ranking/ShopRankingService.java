...
public class ShopRankingService {
    ...
    private boolean matches(Shop s, AgentModels.Intent i) {
        ...
        if (Boolean.TRUE.equals(i.getNeedVoucher()) && s.getId() == null) return false;
        return true;
    }
    ...
    private double score(AgentModels.ShopCard c) { return (c.getScore()==null?0:c.getScore()/5)*.4 + (c.getDistanceMeter()==null?0:1.0/(1+c.getDistanceMeter()/1000))* .3 + (c.getAveragePrice()==null?0:.3); }
}
