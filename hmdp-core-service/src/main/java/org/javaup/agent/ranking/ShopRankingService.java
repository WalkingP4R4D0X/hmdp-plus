...
public class ShopRankingService {
    ...
    private boolean openNow(String hours, String at) { if (hours == null) return false; try { String[] p = hours.split("-"); LocalTime t = LocalTime.parse(at == null ? LocalTime.now().toString().substring(0,5) : at); LocalTime start = LocalTime.parse(p[0].trim()); LocalTime end = LocalTime.parse(p[1].trim()); return start.compareTo(end) <= 0 ? !t.isBefore(start) && !t.isAfter(end) : !t.isBefore(start) || !t.isAfter(end); } catch (Exception e) { return false; } }
