...
const json = JSONbig({ storeAsString: true, protoAction: 'ignore', constructorAction: 'ignore' })
...
export function createSSEParser(onEvent) {
  ...
  function line(value) {
    if (value === '') {
      if (data.length) onEvent({ event: event || 'message', id, data: data.join('\n') })
      event = ''
      id = ''
      data = []
      return
...
export function createAssistant() {
  return {
    role: 'assistant', content: '', cards: [], filters: {}, phase: 'loading',
    fallback: false, errorCode: null, memorySaved: null, noResult: false,
    conversationId: null, traceId: null
  }
}

export function errorMessage(code) {
  return {
    AGENT_NO_LOCATION: '未获取到定位，请允许定位，或补充所在区域、按关键词搜索。',
    AGENT_GEO_INDEX_EMPTY: '附近商户索引正在准备中，请稍后重试。',
    AGENT_GEO_UNAVAILABLE: '附近检索暂时不可用，请稍后重试或按区域、关键词搜索。',
    AGENT_RATE_LIMITED: '请求过于频繁，请稍后再试。',
    AGENT_UNAUTHORIZED: '登录状态或会话已失效，请重新登录或开始新会话。',
    AGENT_REQUEST_INVALID: '请输入 1–500 字的查店需求，包含品类、区域或预算。',
...
export function createAgentReducer(message) {
  ...
  return (frame) => {
    ...
    const structured = ['filter_update', 'shop_card', 'done'].includes(frame.event)
    ...
    if (frame.event === 'done') {
      ...
      if (data) {
        ...
        message.memorySaved = typeof data.memorySaved === 'boolean' ? data.memorySaved : null
      ...
      const nonFatal = ['AGENT_NO_RESULT', 'AGENT_FALLBACK'].includes(message.errorCode)
      message.phase = message.errorCode && !nonFatal ? 'error' : 'done'
      message.noResult = message.phase === 'done' && message.cards.length === 0 && (
        message.errorCode === 'AGENT_NO_RESULT' || Object.values(message.filters).some(v => v != null && v !== '')
      )
      ended = true
    }
    return true
...
export async function consumeAgentStream(response, { signal, active, message }) {
  if (!active()) return
  if (!response.ok) {
    throw new Error(({ 401: 'AGENT_UNAUTHORIZED', 403: 'AGENT_UNAUTHORIZED', 429: 'AGENT_RATE_LIMITED', 400: 'AGENT_REQUEST_INVALID' })[response.status] || 'AGENT_NETWORK_ERROR')
  }
  ...
  try {
    while (active() && !signal.aborted && !finished) {
      ...
      if (done) {
        ...
        parser.finish()
        if (!finished && message.phase !== 'error') throw new Error('AGENT_STREAM_INCOMPLETE')
        break
      }
...
export function createAgentRunner({ stream, stop, onLoading, onConversation }) {
  ...
  return {
    async run(message, payload) {
      if (current) return
      const request = { id: payload.clientRequestId, controller: new AbortController(), message }
      current = request
      onLoading(true)
      ...
      try {
        ...
        await consumeAgentStream(response, { signal: request.controller.signal, active, message })
        if (active() && message.memorySaved === false) {
          onConversation(null)
        } else if (active() && message.phase === 'done') {
          onConversation(message.memorySaved === false ? null : message.conversationId)
        } else if (active() && message.errorCode === 'AGENT_UNAUTHORIZED') {
          onConversation(null)
        }
      } catch (error) {
        if (active() && !request.controller.signal.aborted) {
          message.errorCode = error.message?.startsWith('AGENT_') ? error.message : 'AGENT_NETWORK_ERROR'
          message.phase = 'error'
    ...
    cancel() {
      ...
      onLoading(false)
      if (request.id) Promise.resolve().then(() => stop(request.id)).catch(() => {})
    }
  }
}
  ...
  return new Promise(resolve => {
    ...
    try {
      geolocation.getCurrentPosition(({ coords }) => {
        ...
        resolve(Number.isFinite(latitude) && Math.abs(latitude) <= 90 &&
          Number.isFinite(longitude) && Math.abs(longitude) <= 180 ? { latitude, longitude } : null)
      }, () => resolve(null), { enableHighAccuracy: false, timeout: 5000, maximumAge: 300000 })
    } catch {
      resolve(null)
    }
  })
...
export function filterLabels(filters = {}) {
  const labels = {
    keyword: ['关键词', ''], location: ['区域', ''], radiusMeter: ['距离以内', '米'],
    budgetMax: ['人均上限', '元'], minScore: ['最低评分', '分'], openAt: ['营业时间', ''],
    scene: ['场景', ''], needVoucher: ['优惠券', '']
  }
  return Object.entries(labels).flatMap(([key, [name, unit]]) => {
    const value = filters[key]
    if (value == null || value === '') return []
    return [`${name}：${key === 'needVoucher' ? (value ? '需要' : '不限') : `${value}${unit}`}`]
  })
}
...
export function businessStatus(value) {
  return value === true ? '营业中' : value === false ? '未营业' : '营业状态：信息缺失'
}

export function voucherMoney(value) {
  return value == null || value === '' || !Number.isFinite(Number(value))
    ? '信息缺失' : `${(Number(value) / 100).toFixed(2)} 元`
}
